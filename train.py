...
def main():
    ...
    label_trainloader,unlabel_trainloader, testloader_dict = dm.return_dataloaders()
    ...
    ema_model=models.init_model(name=args.arch, num_classes=dm.num_train_pids, loss={'xent'}, use_gpu=use_gpu, args=vars(args))
    print(ema_model)
    print("ema_Model size: {:.3f} M".format(count_num_param(ema_model)))
    ...
    if args.load_weights and check_isfile(args.load_weights):
        ...
        pretrain_dict_1 = checkpoint['state_dict']
        model_dict = model.state_dict()
        ema_model_dict = ema_model.state_dict()

        pretrain_dict = {k: v for k, v in pretrain_dict_1.items() if k in model_dict and model_dict[k].size() == v.size()}
        ema_pretrain_dict = {k: v for k, v in pretrain_dict_1.items() if k in ema_model_dict and ema_model_dict[k].size() == v.size()}
        model_dict.update(pretrain_dict)
        ema_model_dict.update(ema_pretrain_dict)
    ...
    if args.resume and check_isfile(args.resume):
        ...
        ema_state.update(checkpoint['ema_state_dict'])
    ...
    if args.fixbase_epoch > 0:
        ...
        for epoch in range(args.fixbase_epoch):
            ...
            train(epoch, model,ema_model, criterion, regularizer, optimizer, label_trainloader,unlabel_trainloader, use_gpu, fixbase=True)
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        train(epoch, model,ema_model, criterion, regularizer, optimizer, label_trainloader,unlabel_trainloader, use_gpu, fixbase=False)
        train_time += round(time.time() - start_train_time)
...
def train(epoch, model,ema_model, criterion, regularizer, optimizer, label_trainloader,unlabel_trainloader, use_gpu, fixbase=False):
    ...
    if fixbase or args.fixbase:
        ...
        open_specified_layers(ema_model, args.open_layers)
    else:
        open_all_layers(model)
        open_all_layers(ema_model)

    end = time.time()
    x=[]
    for i, (uninput,_, _, _) in enumerate(unlabel_trainloader):
        try:
            limited = float(os.environ.get('limited', None))
        except (ValueError, TypeError):
            limited = 1

        if not fixbase and (i + 1) > limited * len(unlabel_trainloader):
            break
        if use_gpu:
    ...
    for j, (input,target, _, _) in enumerate(label_trainloader):
        ...
        if not fixbase and (j + 1) > limited * len(label_trainloader):
            break
        if use_gpu:
            input = input.cuda()
            target = target.cuda()
        unlabelinput=x[random.randint(0,len(x)-1)]
        if use_gpu:
        ...
        criteria = nn.MSELoss()
        loss2 = criteria(unlabeloutput_1[0],unlabeloutput_2[0])
        loss3 = criteria(output[0], labeloutput_2[0])
        ...
        if (j+1) % args.print_freq == 0 :
                print('label_Epoch: [{0}][{1}/{2}]\t'
                          'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                          'Data {data_time.val:.4f} ({data_time.avg:.4f})\t'
                          'Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(
                            epoch+1 , j+1, len(label_trainloader),batch_time=batch_time,
