...
class BaseDataManager(object):
    ...
    def return_dataloaders(self):
        ...
        """
        return self.label_trainloader, self.unlabel_trainloader, self.testloader_dict
...
class ImageDataManager(BaseDataManager):
    ...
    def __init__(self,
        ...
        self._num_untrain_pids = 0
        self._num_untrain_cams = 0
        ...
        for name in self.source_names:
            ...
            for img_path, pid, camid in dataset.label_train:
                pid += self._num_train_pids
                camid += self._num_train_cams
                self.label_train.append((img_path, pid, camid))

            self._num_train_pids += dataset.num_train_pids
            self._num_train_cams += dataset.num_train_cams
            for img_path, pid, camid in dataset.unlabel_train:
                pid += self._num_untrain_pids
                camid += self._num_untrain_cams
                self.unlabel_train.append((img_path, pid, camid))

            self._num_untrain_pids += dataset.num_untrain_pids
            self._num_untrain_cams += dataset.num_untrain_cams

        if self.train_sampler == 'RandomIdentitySampler':
            print('!!! Using RandomIdentitySampler !!!')
            self.label_trainloader = DataLoader(
                ImageDataset(self.label_train, transform=transform_train),
                sampler=RandomIdentitySampler(self.label_train, self.train_batch_size, self.num_instances),
                batch_size=self.train_batch_size, shuffle=False, num_workers=self.workers,
                pin_memory=self.pin_memory, drop_last=True
            )
            self.unlabel_trainloader = DataLoader(
                ImageDataset(self.unlabel_train, transform=transform_train),
                sampler=RandomIdentitySampler(self.unlabel_train, self.train_batch_size, self.num_instances),
                batch_size=self.train_batch_size, shuffle=False, num_workers=self.workers,
                pin_memory=self.pin_memory, drop_last=True
            )
        ...
        print("  # label_train images   : {}".format(len(self.label_train)))
        print("  # unlabel_train images   : {}".format(len(self.unlabel_train)))
